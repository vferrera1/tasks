...
export function isCorrect(question: Question, answer: string): boolean {
    const uncappedQuestion: string = question.expected.toLowerCase().trim();
    const uncappedAnswer: string = answer.toLowerCase().trim();
    return uncappedQuestion === uncappedAnswer;
}
...
export function toMarkdown(question: Question): string {
    const firstLine: string = "# " + question.name;
    const secondLine: string = question.body;
    if (question.type === "multiple_choice_question") {
        const choices: string = question.options.reduce(
            (choiceList: string, option: string) =>
                choiceList + "\n- " + option,
            ""
        );
        return firstLine + "\n" + secondLine + choices;
...
export function publishQuestion(question: Question): Question {
    const updatedQuestion = { ...question, published: !question.published };
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const oldQuestionCopy = {
        ...oldQuestion,
        id: id,
        name: "Copy of " + oldQuestion.name,
        published: false
    };
    return oldQuestionCopy;
}
